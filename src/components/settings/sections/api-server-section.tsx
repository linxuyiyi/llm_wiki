...
export function buildSourceCurlExamples(token: string) {
  return {
    put: `curl -X PUT \\
  ...
  -H "Authorization: Bearer ${token}" \\
  "${API_SERVER_BASE_URL}/api/v1/projects/current/sources/file?path=source-demo.md"`,
  }
}
...
export function ApiServerSection({ draft, setDraft }: Props) {
  ...
  const sourceCurlExamples = useMemo(
    () => buildSourceCurlExamples(sourceTokenForExample),
  ...
  const handleCopySourcePutCurl = useCallback(async () => {
    try {
      ...
      console.error("[api-settings] copy source PUT curl failed:", err)
    }
  }, [sampleSourcePutCurl])
  ...
  const handleCopySourceDeleteCurl = useCallback(async () => {
    try {
      ...
      console.error("[api-settings] copy source DELETE curl failed:", err)
    }
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">
            {t("settings.sections.apiServer.openHealthHint", {
              defaultValue: "/health never requires authentication. Other endpoints follow the access mode below, except Agent chat, cancellation, page embedding, and source file mutations, which always require a token.",
            })}
          </span>
        </div>
      </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            {t("settings.sections.apiServer.tokenHint", {
              defaultValue:
                "Send as `Authorization: Bearer <token>` or `X-LLM-Wiki-Token: <token>`. Read-oriented endpoints may omit it when unauthenticated access is enabled, but Agent chat, cancellation, page embedding, and source file mutations always require it. The environment variable LLM_WIKI_API_TOKEN overrides this field if set.",
            })}
          </p>
        </div>
          ...
          {API_ENDPOINTS.map((endpoint) => {
            ...
            return (
              ...
              >
                ...
                {endpoint.path === "/api/v1/projects/{id}/sources/file" && endpoint.method === "PUT" && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={handleCopySourcePutCurl}
                    disabled={hasUnsavedApiConfig}
                ...
                {endpoint.path === "/api/v1/projects/{id}/sources/file" && endpoint.method === "DELETE" && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={handleCopySourceDeleteCurl}
                    disabled={hasUnsavedApiConfig}
                    className="ml-auto h-6 gap-1 px-2 text-[10px]"
                  >
                    <Copy className="h-3 w-3" />
